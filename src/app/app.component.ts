...
export class AppComponent {
  ...
  constructor(private renderer: Renderer2,
    ...
    private _router: Router,@Inject(DOCUMENT) private document: Document, private translationService: TranslationService) { }
  ngOnInit(): void {


    this.primengConfig.ripple = true;

    this.currentLang =  localStorage.getItem('currentLang')  || environment.defaultLanguage;
...
this.constantsStoreSer.getData()
  ...
  changeCurrentLang(lang) {
    ...
    let newRoute = this._router['location']['_platformLocation']['location']['pathname'].replace(/^.{3}/g, `${lang}` );
